...
def inverse_warping(img, left_cam, right_cam, depth):
    ...
    filler = torch.Tensor([0.0, 0.0, 0.0, 1.0]).to(depth.device).reshape(1, 1, 4)  # [1, 1, 4]
    ...
    grid = _meshgrid_abs(img_height, img_width).to(depth.device)  # [3, height * width]
    ...
    ones = torch.ones([batch_size, 1, img_height * img_width], device=depth.device)  # [batch_size, 1, height * width]
    cam_coords_hom = torch.cat([cam_coords, ones], dim=1)  # [batch_size, 4, height * width]

    hom_filler = torch.Tensor([0.0, 0.0, 0.0, 1.0]).to(depth.device).reshape(1, 1, 4)  # [1, 1, 4]
    hom_filler = hom_filler.repeat(batch_size, 1, 1)  # [B, 1, 4]
    intrinsic_mat_hom = torch.cat([K_left.float(), torch.zeros([batch_size, 3, 1], device=depth.device)], dim=2)  # [B, 3, 4]
    ...
    return warped_right.permute(0, 3, 1, 2), mask.permute(0, 3, 1, 2)
...
def _cam2pixel(cam_coords, proj_c2p):
    ...
    pixel_coords = pcoords[:,:2,:] / (pcoords[:,2:3,:] + 1e-10)
